<template>
	<view>
		...
		<view class="uni-container">
			<unicloud-db ref="udb" collection="jz-custom-yqm,uni-id-users"
				field="user_type,user_name_type,user_name,date,value,status,user_id.nickname" :where="where"
				...
				<uni-table ref="table" :loading="loading" :emptyText="error.message || '没有更多数据'" border stripe
					...
					<uni-tr>
						<uni-th align="center" filter-type="search" @filter-change="filterChange($event, 'user_id')"
							sortable @sort-change="sortChange($event, 'user_id')">申请人</uni-th>
						<uni-th align="center" filter-type="select"
							:filter-data="options.filterData.user_type_localdata"
							@filter-change="filterChange($event, 'user_type')">使用状态</uni-th>
						...
						<uni-th align="center" filter-type="search" @filter-change="filterChange($event, 'value')"
							sortable @sort-change="sortChange($event, 'value')">邀请码</uni-th>
					...
					<uni-tr v-for="(item,index) in data" :key="index">
						<uni-td align="center">{{(item.user_id&&item.user_id.length)>0?item.user_id[0].nickname:''}}
						</uni-td>
						<uni-td align="center">{{options.user_type_valuetotext[item.user_type]}}</uni-td>
						...
						<uni-td align="center">
							<checkbox-group @change="change_data(item,'status')">
								<checkbox value="status" :checked="item.status == true" />
							</checkbox-group>
						</uni-td>
						...
						<uni-td align="center">
							<view class="uni-group">
								<button v-show="!item.value" @click="createQym(item)" class="uni-button" size="mini"
									type="primary">生成邀请码</button>
						...
						if (res.confirm) {
							...
							for (var i = 0; i < number; i++) {
								add_value.push({
									status: true,
									user_type: 1,
									value: Math.random().toString(36).substr(2) + Math.random()
										.toString(36).substr(2)
								})
							}
							return db.collection(dbCollectionName).add(add_value).then((res) => {
								uni.showToast({
									title: '新增成功'
								});
								if (this.$refs.udb) {
