  ...
  <view class="uni-container">
    <uni-forms ref="form" :value="formData" validateTrigger="bind">
     <uni-forms-item label="群昵称">
     	<lauwen-select :options="options" :defaultIndex="defaultIndex" :height="45" :padding="15" :fontSize="'1rem'"
	 ...
	 <!-- <uni-forms-item name="three_name" label="三审负责人">
	  	<lauwen-select :options="three_options" :defaultIndex="defaultIndex1" :height="45" :padding="15" :fontSize="'1rem'"
	  		@getValue="getValue_three"></lauwen-select>
	  
	  </uni-forms-item> -->
      <view class="uni-button-group">
        <button type="primary" class="uni-button" style="width: 100px;" @click="submit">提交</button>
        <button class="uni-button" style="width: 100px;" @click="cancel">返回</button>
      </view>
    </uni-forms>
  </view>
</template>
...
<script>
  import { validator } from '../../js_sdk/validator/jz-custom-shtime.js';
...
cancel() {
      ...
      getDetail(id) {
        ...
        db.collection(dbCollectionName).doc(id).field("user_name,name,start_date,end_date,three,three_name").get().then((res) => {
          const data = res.result.data[0]
          if (data) {
			  // debugger;
            this.formData = data;
			this.defaultIndex=data.name;
			this.defaultIndex1=data.three;
          }
        }).catch((err) => {
          uni.showModal({
            content: err.message || '请求服务失败',
