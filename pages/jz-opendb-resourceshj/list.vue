		...
		<view class="uni-container">
					...
					<uni-tr v-for="(item,index) in data" :key="index">
						...
						<uni-td align="center">
							<view class="uni-group">
								...
								<button @click="openhjgl(item.hj_id)" class="uni-button" size="mini" type="warn">合集管理</button>
			...
			<u-modal v-model="showaddhjzy" width="600px" @cancel="showaddhjzy==false" :show-cancel-button="true"
				...
				<view class="uni-container" style="height: 500px;overflow: auto;">
					<unicloud-db ref="listudb" collection="jz-opendb-resources" field="title,zy_gs" :where="listwhere"
						page-data="replace" orderby="publish_date desc" :getcount="true" :page-size="100"
			...
			<u-modal v-model="showhjManage" width="600px" @cancel="showhjManage==false" :show-cancel-button="true"
				title="合集管理" @confirm="addhjzy" confirm-text="添加">
				<view class="uni-container" style="height: 500px;overflow: auto;">
					<uni-table ref="table" emptyText="'没有更多数据'" border stripe>
						<uni-tr>
						...
						<uni-tr v-for="(item,index) in hjdata" :key="index">
							...
							<uni-td align="center">{{zy_gs_localdata[item.article_id[0]?item.article_id[0].zy_gs:''].text}}</uni-td>
		...
		methods: {
			...
			async openhjgl(hj_id) {
				...
				var res = await db.collection("jz-opendb-resourceshj,jz-opendb-resources").where({
					"parent_id": hj_id
				}).field("resourceshj_title,sort,article_id{title,zy_gs},hj_id").get();
				console.log("res", res);
				...
				dataList.forEach((item, index) => {
					if (this.selectedIndexs.indexOf(index) != -1) {
						list.push({
							parent_id: hj_id,
							hj_id: hj_id + index,
							sort: parseInt(item.sort) || ++sortinit,
							article_id: item._id
						});
					}
				});
