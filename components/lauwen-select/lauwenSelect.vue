	...
	<view>
		<view class="lauwen-select">
			<view class="lauwen-select-box">
				<view :style="{padding: padding+'rpx', fontSize:fontSize}" class="lauwen-select-input"
					@tap="lauwenSelect()">
					<view class="lauwen-select-input-text" :style="{height: height+'rpx', lineHeight: height+'rpx'}"
						:data-index="selected_index">{{ options[selected_index] || "请选择" }}</view>
			...
			<view class="lauwen-options" v-show="is_selecting">
				<view v-for="(item, index) in options" class="lauwen-option"
					:class="selected_index == index ? 'lauwen-option-active' : ''" :key="index"
					@tap="lauwenSelected(index)"
					:style="{height: (height + padding*2)+'rpx', lineHeight: height+'rpx', padding: padding+'rpx', fontSize:fontSize}">
					{{ item }}</view>
			</view>
		</view>
	</view>
...
<script>
	export default {
		...
		watch:{
			defaultIndex(){
				this.selected_index=this.defaultIndex;
			}
		},
		methods: {
			lauwenSelect() {
